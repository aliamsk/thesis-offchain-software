...
use ergo_offchain_utilities::encoding::{unwrap_hex_encoded_string, unwrap_int, unwrap_long, unwrap_string};
use std::path::Path;
...
impl OraclePool {
    ...
    pub fn get_live_epoch_state(&self) -> Result<LiveEpochState> {
        ...
        let latest_pool_datapoint = unwrap_string(&epoch_box_regs[0])?;
        ...
        let epoch_state = LiveEpochState {
            ...
            latest_pool_datapoint: latest_pool_datapoint.to_string(),
        };
    ...
    pub fn get_preparation_state(&self) -> Result<PreparationState> {
        ...
        let latest_pool_datapoint = unwrap_string(&epoch_prep_box_regs[0])?;
    ...
    pub fn get_datapoint_state(&self) -> Result<DatapointState> {
        ...
        let datapoint = unwrap_string(&datapoint_box_regs[2])?;
