...
impl Connector {
    ...
    pub fn new(
        title: &str,
        get_datapoint: fn() -> Result<String>,
    ...
    pub fn check_bootstrap(&self) {
        ...
        if args.len() > 1 && &args[1] == "--bootstrap-value" {
            if let Ok(price) = (self.get_datapoint)() {
                ...
                let constant: Constant = (price as str).into();
    ...
    pub fn run(&self) {
        ...
        loop {
            ...
            // Otherwise if state is accessible
                ...
                println!("repost check after unwrap: {}", repost_check);
                let should_post = repost_check
                    || &pool_status.current_pool_stage == "Live Epoch"
                        && oracle_status.waiting_for_datapoint_submit;

                if should_post {
                    let mut price_res = (self.get_datapoint)();
                    println!("In connector run loop, sending datapoint: {}", price_res);
                    // If acquiring price worked
                    if let Ok(price) = price_res {
                        println!("Posting price in connector: {}", price);
...
impl Connector {
    ...
    pub fn new_basic_connector(title: &str, get_datapoint: fn() -> Result<String>) -> Connector {
