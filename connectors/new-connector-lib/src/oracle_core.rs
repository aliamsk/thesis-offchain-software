...
impl OracleCore {
    ...
    pub fn submit_datapoint(&self, datapoint: String) -> Result<String> {
        ...
        if let Ok(resp_json) = json::parse(&resp_text) {
            let mut tx_id = resp_json["tx_id"].clone();

            // If there no tx_id/there is an error
            if tx_id.is_empty() {
                let mut error = resp_json["error"].clone();
                println!("Custom log...submitting datapoint error: ", error.to_string());
                return Err(ConnectorError::FailedSubmittingDatapoint(error.to_string()));
            } else {
                println!("I did it...Datapoint submitted successfully...tx_id: {}", tx_id.to_string());
    ...
    pub fn oracle_status(&self) -> Result<OracleStatus> {
        ...
        println!("getting oracle status in oracle core: {}", resp_text);
        from_str(&resp_text).map_err(|_| ConnectorError::FailedParsingCoreResponse(resp_text))
    }
    ...
    pub fn pool_status(&self) -> Result<PoolStatus> {
        ...
        println!("getting pool status in oracle core: {}", resp_text);
        from_str(&resp_text).map_err(|_| ConnectorError::FailedParsingCoreResponse(resp_text))
    }
    ...
    pub fn repost_check(&self) -> Result<bool> {
        ...
        println!("repost check response: {}", resp_text);
        resp_text
            .parse()
            .map_err(|_| ConnectorError::FailedParsingCoreResponse(resp_text))
    }
